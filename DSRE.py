...
# v1.15: 高層 decay 補正 (滑らか slope 化)
# 旧 v1.13/v1.14 は i>=5 に一律 1.5x の段差ブースト → 32-48k 帯にスペクトル
# 局所ピークを生む副作用があった。v1.15 は段差ではなく層 i に対する連続関数
# `boost(i) = max(1.0, base - slope*(i - start))` で配分し、隣接層との段差を
# 解消する (補完量総和はほぼ同等に維持)。
HARMONIC_HIGH_LAYER_BOOST_BASE = 1.7   # i=start の最大ブースト
HARMONIC_HIGH_LAYER_BOOST_SLOPE = 0.18  # i ごとの線形低下幅
HARMONIC_HIGH_LAYER_BOOST_START = 4    # ブースト開始 layer (0-indexed)

# --- v1.15: Spectral tilt (補完成分を高域ほど自然減衰させる) ---
# 各 path の出力に簡易 high-shelf 1-pole IIR で HF tilt を加える。
# slope は概ね -3dB/oct。総エネルギーはわずかに下がるが、隣接帯域の段差が
# 平滑化され、Audacity スペクトルで単調減衰の自然なカーブになる。
SPECTRAL_TILT_REF_HZ = 8000        # tilt 軸 (この周波数を境に高域減衰)
SPECTRAL_TILT_SLOPE = 0.30         # 1-pole 係数 (大きいほど tilt 急、補完量を削らない範囲)
...
class DSREParams:
    ...
    # v1.15: 高層 decay 補正 (滑らか slope 化、局所ピーク解消)
    high_layer_boost_base: float = HARMONIC_HIGH_LAYER_BOOST_BASE
    high_layer_boost_slope: float = HARMONIC_HIGH_LAYER_BOOST_SLOPE
    high_layer_boost_start: int = HARMONIC_HIGH_LAYER_BOOST_START
    # v1.15: spectral tilt (補完成分を高域ほど自然減衰)
    tilt_ref_hz: int = SPECTRAL_TILT_REF_HZ
    tilt_slope: float = SPECTRAL_TILT_SLOPE
...
def _apply_hf_tilt(sig: np.ndarray, sr: int, ref_hz: float, slope: float) -> np.ndarray:
    """v1.15: 簡易 high-shelf 1-pole tilt で高域ほど自然減衰させる。

    1 次 IIR ローパスを参照周波数に置き、原信号と差し引いて high-shelf を作る:
      out = sig - slope * lowpass(sig, ref_hz)
    slope=0 で無処理、slope>0 で高域減衰 (低域ほど無処理)。
    SOS で実装し sosfiltfilt は使わない (位相 0 不要、軽量化優先)。
    """
    if slope <= 0.0 or sig.size == 0:
        return sig
    nyq = sr / 2.0
    wn = max(1e-6, min(0.999, ref_hz / nyq))
    sos_lp = signal.butter(2, wn, btype="lowpass", output="sos")
    try:
        lp = signal.sosfilt(sos_lp, sig, axis=-1).astype(sig.dtype, copy=False)
    except Exception:
        return sig
    if not np.all(np.isfinite(lp)):
        return sig
    out = sig - slope * (sig - lp)  # = (1-slope)*sig + slope*lp = LP-shelf with high-cut
...
def harmonic_exciter(x, sr, params: "DSREParams | None" = None):
    ...
    # v1.15: 高域 tilt を適用してスペクトル単調減衰化 (隣接帯域段差解消)
    out = _apply_hf_tilt(out, sr, p.tilt_ref_hz, p.tilt_slope)
...
def air_band_sparkle(x, sr, params: "DSREParams | None" = None):
    ...
    # v1.15: 高域 tilt で自然減衰 (隣接帯域との段差解消)
    out = _apply_hf_tilt(out, sr, p.tilt_ref_hz, p.tilt_slope)
    return out.astype(x.dtype, copy=False)
...
def nyquist_complement(x, sr, params: "DSREParams | None" = None):
    ...
    # v1.15: 高域 tilt で 32-48kHz 内の自然減衰を保つ (フラットな塊を防止)
    out = _apply_hf_tilt(out, sr, p.tilt_ref_hz, p.tilt_slope)
    return out.astype(x.dtype, copy=False)
...
def zansei_impl(x, sr, progress_cb=None, abort_cb=None):
    ...
    # v1.15: 段差ブースト (旧 1.5x 一律) を連続関数に置換し、隣接層との
    # スペクトル段差を解消。base * (1 - slope*(i-start)) で滑らかに減少、
    # 1.0 を下回らないよう clamp (補完量低下を防ぐ)。総和は v1.13 同等。
    base = PARAMS.high_layer_boost_base
    slope_b = PARAMS.high_layer_boost_slope
    start = PARAMS.high_layer_boost_start
    for i in range(start, n_layers):
        bf = max(1.0, base - slope_b * (i - start))
        decays[i] = decays[i] * bf
    nyq = sr / 2.0
